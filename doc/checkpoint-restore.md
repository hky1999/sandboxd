...
The relisting alone still leaves windows open that cached verdicts cannot close, so the sweep and the publishers contend for an exclusive `gc-claims/<key>` whose ENTIRE lifecycle is generation-bound: the claim body carries a per-run unique owner nonce (a fixed string is no credential), creation is an atomic conditional write (`If-None-Match:*`; local stores use `O_EXCL`), and every release or stale cleanup is a conditional DELETE bound to the observed body's ETag — a claim another participant created in the meantime can never be destroyed by an older observation.

Every write path that makes an object matter participates. A Has-hit reuse probes the mark and, when marked, claims before re-uploading and releases after. A Has-MISS recreate (a collector may have deleted the object seconds ago mid-claim) gets the same treatment after the upload: the mark is probed, and a marked key is re-uploaded once more under the claim, so the object provably exists when the claim is released. The sweep runs claim-first in three phases: it wins the claims of all candidates, then re-derives the deletion basis UNDER the claims (one fresh listing; artifact sets that appeared or changed get their references resolved — a publication that committed since the sweep's first observation spares its objects even with min-age disabled), and only then acts, with each verdict taken from a fresh HEAD of the object rather than any cached listing. A lost claim race (412) spares the object unconditionally; a publisher meeting a sweep's held claim waits it out (bounded, then fails closed) and re-uploads under its own claim once freed. Claims a crashed participant never released are cleared as stale past the same grace bound — fresh observation, still grace-old, generation-bound delete.

Conditional CREATE is mandatory: a store whose writes are not create-only cannot host the claim protocol at all, and the sweep's preflight probe refuses to delete anything there; the claim create also verifies itself by read-back so a server that silently overwrites fails loud instead of granting two winners. Conditional DELETE (If-Match) is opportunistic: stores that honor it get an atomic confirm-then-delete for claim cleanup, and stores that ignore the header degrade safely — the cleanup re-observes the claim fresh and requires it to still be grace-old before the delete, and inside the documented grace bound a grace-old claim has no live releaser (live owners release within milliseconds), so the residual collapses into the same long-publish boundary. With the claim lifecycle in place the residual exposure reduces to a publish that outlives the full grace window with no surviving claim (crashed publisher); keep the grace at or above the longest legitimate publish (cn-publishd crash-resume included). Concurrent sweeps are safe: marks and claims are idempotent, deletions are idempotent, and the reference scan is read-only.
