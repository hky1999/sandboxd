...
The relisting alone still leaves one window open: a publisher can refresh an object and commit its INDEX entirely between the sweep's relisting and its DELETE, and the sweep would act on the cached verdict. Both sides therefore contend for an exclusive `gc-claims/<key>` created atomically with a conditional write (`If-None-Match:*`; local stores use `O_EXCL`): a fenced publisher wins the claim before its re-upload and holds it past the INDEX commit, and the sweep must win the claim after its relisting, immediately before the delete — a lost claim race (412) spares the object unconditionally, and a sweep holding the claim makes the publisher wait it out (bounded, then fail-closed) and re-upload under its own claim once freed. Claims a publisher never released are cleared as stale past the same grace bound. Stores that do not honor conditional writes disable deletion entirely (the sweep proves the primitive with a preflight probe before touching anything); the claim create verifies itself by read-back so a server that silently overwrites fails loud instead of granting two winners. With claims in place the residual exposure reduces to a publisher whose Has probes all precede the marking yet whose publish outlives the full grace window; keep the grace at or above the longest legitimate publish (cn-publishd crash-resume included). Concurrent sweeps are safe: marks and claims are idempotent, deletions are idempotent, and the reference scan is read-only.
