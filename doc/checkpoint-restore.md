...
Content-addressed chunk objects are shared across artifacts and generations, so they cannot expire by age. `cn-gcsweep -store URL` reference-counts instead: every surviving artifact's INDEX and the sidecars it names (bundle or per-file) contribute the exact live key set — plain and `.z` chunk objects under `<aa>/<digest>` keys, memory packs of both identities, overlay chunks under their own `overlay-chunks/<aa>/<digest>` namespace — and only objects referenced by no surviving artifact become deletion candidates, additionally guarded by `-min-age` (default 24h) — and, decisively, by the publication intents below — so chunks of an in-flight publication, whose objects land before their INDEX, are never caught mid-publish. Artifact sets (`artifacts/<id>/*`) are ID-named and exclusive, so they may age out wholesale: `-max-artifact-age` drops sets whose INDEX is older and `-drop ID,ID` names sets explicitly; a chunk dies only when its last referencing artifact is gone. The tool is fail-closed — one unfetchable or undecodable INDEX or sidecar aborts every deletion because the live set cannot be proven — and objects with unrecognized key shapes are never touched. Without `-delete` the run only reports and writes nothing.

Deletion is mark-then-sweep because min-age alone cannot make a single-pass delete safe: a publisher that reuses an OLD object (unchanged chunks are Has hits, never re-uploaded) publishes no young bytes for the sweep to spare, and its INDEX lands last. A candidate is therefore recorded as a `gc-marks/<key>` object and collected only once that mark has aged past `-mark-grace` (default 24h). The grace window is fenced from both sides: publishers probe the mark at every Has-hit reuse and refresh the object by re-uploading instead of skipping (an inherited hole has no local bytes, so a marked parent fails the publish with a republish-the-parent error instead of sealing zeros under its digest), and the sweep re-lists the bucket immediately before deleting — an object that vanished, was refreshed, or is referenced by an artifact set that appeared mid-sweep is spared and its mark cleared. Artifact-set drops re-verify the set's INDEX mtime the same way.

The relisting alone still leaves windows open that cached verdicts cannot close, so the sweep and the publishers contend for an exclusive `gc-claims/<key>` whose ENTIRE lifecycle is generation-bound: the claim body carries an owner nonce that is unique PER ACQUISITION (a fixed string is no credential, and neither is a per-process one — two acquisitions of the same key by one process must not share an ETag either, or a conditional delete bound to the earlier acquisition would also match the later claim), creation is an atomic conditional write (`If-None-Match:*`; local stores use `O_EXCL`), and every release or stale cleanup is a conditional DELETE bound to the observed body's ETag — a claim another participant created in the meantime can never be destroyed by an older observation. On local stores the conditional delete is a real cross-process compare-and-delete: the read-compare-unlink runs under an exclusive flock on the object with the path re-resolved to the same inode after the lock, so a claim unlinked and re-created between the open and the lock is detected and left alone.

Every write path that makes an object matter participates. A Has-hit reuse probes the mark and, when marked, claims before re-uploading and releases after. A Has-MISS recreate (a collector may have deleted the object seconds ago mid-claim) gets the same treatment after the upload: the mark is probed, and a marked key is re-uploaded once more under the claim, so the object provably exists when the claim is released. The sweep runs claim-first in three phases: it wins the claims of all candidates, then re-derives the deletion basis UNDER the claims, and only then acts, with each verdict taken from a fresh HEAD of the object rather than any cached listing. A lost claim race (412) spares the object unconditionally; a publisher meeting a sweep's held claim waits it out (bounded, then fails closed) and, once the claim frees, re-verifies and re-uploads under its own claim — a publisher that meets an already-won collector never proceeds on a pre-contention observation. Mark mutation is claim-guarded on every path: the verdicts clear marks under claims they already hold, and the obsolete-mark pass takes the key's claim before clearing (skipping on a lost race), so no mark is ever removed while another collector holds the object — a mark removed under a parked collector's claim would let a later publisher reuse the object fence-less, straight into the collector's resumed DELETE. Claims a crashed participant never released are cleared as stale past `-claim-grace` — its own knob, deliberately decoupled from `-mark-grace` and positive under `-delete`, because a collector holds its claims across its whole claim-to-verdict batch: a `-mark-grace` of 0 must never let one collector clear another collector's held claim mid-flight, since the first collector's resumed DELETE would then run with the mutual exclusion a fiction (a publication intent cannot prevent this — it protects publishers, not a running collector's own claims). Cleanup is fresh observation, still grace-old, generation-bound delete, and never while any young publication intent exists: a live publisher's claim always sits inside its own intent window. Keep `-claim-grace` at or above the longest legitimate claim-held interval — a full sweep's claim-to-verdict batch (which scales with the candidate count), and a publisher's single fenced upload.

The per-object claims each end at their upload's PUT and cannot cover the release→INDEX gap, so every publication additionally writes a PUBLICATION INTENT before its first store object and removes it after the INDEX commits or on failure. Intents are PER-ATTEMPT — `gc-pubintents/<checkpoint-id>/<attempt-nonce>` — so overlapping attempts under one checkpoint ID never share state: the attempt that finishes first removes only its own intent (a conditional delete bound to its own handle) and cannot strip a still-running sibling's protection, which a single shared key would do however carefully its delete is conditioned. The window covers every object family of the run — Has-reused plain chunks, `.z` bodies, inherited digests with no local bytes, packs, overlay chunks, and the artifact set — because the intent opens before any of them are touched. Establishing the intent is a safety record, not telemetry: a failed intent PUT fails the whole publish closed BEFORE any data upload or INDEX write (the intent END failing is the conservative direction — protection simply outlives the attempt — and stays best-effort), and a cancelled or crashed attempt leaves its intent to expire. While ANY intent under the prefix is younger than `-intent-grace` (default 24h, its own knob, rejected at zero with `-delete`), the sweep deletes NOTHING that run — no data objects, no artifact-set drops, no stale-claim cleanup. That skip is deliberately whole-bucket: a deployment with continuously in-flight publications starves collection until a quiet window, and each crash-leftover intent blocks collection for up to the grace, so the grace bound doubles as the valid-publish-duration boundary — keep it at or above the longest legitimate publish (cn-publishd crash-resume included). Expired intents are cleared exactly like stale claims, each by its listed key: fresh GET for the current ETag and age, require still grace-old, then a conditional delete bound to that observed generation. Deployment restriction: the intent protocol requires every publisher writing to the store to be intent-aware; publishers from before this change write no intent, so sweeps see their release→INDEX gap as unprotected — deploy the new publishers before (or together with) the new sweeps and drain old ones before relying on aggressive (low `-min-age`) collection.

Every deletion basis is derived under an ORDERED observation: the sweep takes a fresh prefix listing of `gc-pubintents/` BEFORE the INDEX listing of the same basis. A publisher commits its INDEX first and deletes its intent second, so an observation that saw no young intent can only mean the intent was already deleted, which implies the INDEX was already committed, which implies the later listing must see it. The reverse order — or a single multi-page listing, whose lexicographic pagination serves `artifacts/` before `gc-pubintents/` — can observe neither and delete a committed publication's chunks. These listings are observations, not atomic mutual exclusion: exclusion is carried by the claims held through the verdicts and by the conditional writes the preflight proves.

Conditional CREATE is mandatory, and so is conditional DELETE. The preflight runs before the sweep mutates anything (marks included) whenever a `-delete` run has any victim, drop, or stale claim to process, and proves both primitives: the second create-only PUT must answer 412, a wrong-ETag DELETE must be refused with the object intact, and the correct-ETag DELETE must execute. A backend that ignores `If-Match` on DELETE (the team MinIO does) turns stale-claim cleanup into a non-atomic compare-and-delete — a delete issued against one observed generation can destroy a freshly created replacement claim — and there is no safe degradation for that store shape, so such a backend is refused automatic deletion outright (mark-only and report runs are unaffected). A backend whose ETags are not the content MD5 of single-part PUTs (SSE-KMS and friends) fails the correct-ETag probe and is refused symmetrically: claims there could never be released or cleaned, only accumulated, and fenced publishes would fail closed after the acquire bound. With the claim lifecycle and the publication intents in place, the residual exposure is a publish that outlives the full intent/claim grace window with no surviving claim or intent (crashed publisher) — the documented long-publish boundary. Concurrent sweeps are safe: marks, claims, and intents are idempotent, deletions are idempotent, and the reference scan is read-only.
