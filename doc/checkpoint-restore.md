...
Incremental checkpoint scheduling (which generations a caller takes and when), deterministic replay, and automatic recovery of a source after checkpoint failure are outside this design. `cn-migrate` orchestrates stop-and-copy migration on top of these primitives: it checkpoints with leave-running=false (the source freezes at the checkpoint instant — no dual-writer window, no lost post-checkpoint writes), uses a fresh immutable checkpoint id per attempt, verifies the target by parsing the listing for the exact id in the RUNNING state, and treats a failed source deletion as a hard failure rather than success. Before checkpointing it pins the incarnation: `sbox inspect` output for exactly that sandbox is parsed strictly as a single `runtime.SandboxStatus` (protojson accepts the number, enum-name, and omitted forms of the state field — an omitted proto3-zero state is a legal RUNNING), the id must match exactly, the state must be RUNNING, and the reserved `akernel.scheduler/resource-generation` label must be present, non-blank, and at most 256 bytes; a node or CLI that cannot produce that identity fails the migration before any checkpoint, and an optional `-source-generation` value must equal the captured label. Both the checkpoint and the source retirement are then conditional on that one captured value (`--expected-generation` through `CheckpointIfGeneration` and the receipt-verified `DeleteIfGeneration`), and a failed source retirement is terminal — reported as `source-cleanup-pending`, never replaced by an unconditional `sbox delete` and never substituted by an empty listing, because absence attests nothing about which incarnation stopped and a bare-ID delete could retire a replacement that reused the id. Compensation is phase-gated: while the target has not been asked to restore anything (publish, placement, or materialization failure), the source is rolled back from its local checkpoint directory; once the restore command has been issued, its outcome can be unknown (a lost or timed-out reply, or a verify that never observes RUNNING), and no observation the orchestrator can make — in particular a delete followed by an absent listing, which does not cancel an in-flight restore — proves the target stayed down, so the orchestrator issues no rollback and no target delete, reports TARGET OUTCOME UNKNOWN with the checkpoint id and target preserved, and leaves recovery to an operator who must first resolve the pending restore or the actual writer on the target; after the target is verified RUNNING it owns the sandbox and the source is only ever deleted, never resurrected. A checkpoint command that fails while the source probe still observes the sandbox RUNNING reports an unknown request outcome rather than claiming the checkpoint was not executed. These rules are a fail-closed containment boundary around unknown target outcomes, not a complete fix: there is still no persistent cross-node ownership record, idempotency token or epoch protocol, no runtime-side rejection of late requests under a controller lease, so a restore that lands after the orchestrator gave up can still start a target next to a manually recovered source, and every TARGET OUTCOME UNKNOWN exit requires a human decision. The generation pinning binds the orchestrator's own two operations to one captured incarnation label; it is not a monotonic writer epoch or a lease — another actor can still start a new source incarnation under the same ID, and there is still no durable migration operation or cross-node fencing.
...
### Packed publication without standalone chunk probes

`cn-publish -pack-mib 4 -pack-skip-chunk-probe` (library `Options.PackSkipChunkProbe`) skips existence checks and reuse for standalone memory chunk objects. The default is false, retaining cross-format chunk reuse. Setting it without packing is rejected before acquiring a publication claim or writing state/objects. Present baseline packs are still checked and reused; missing baseline packs are rebuilt, and new packs are still checked before upload, so retries can reuse completed packs. Zero chunks remain implicit, and payload bounds, per-chunk verification and INDEX-last publication are unchanged. Both pack identities support this option.

This option trades standalone chunk reuse for fewer HEAD requests. When standalone objects already contain the data, it can upload additional pack bytes containing the same content. It does not reduce overlay probes or skip pack verification, and it is not a guarantee of lower checkpoint or migration latency. Measure requests, uploaded bytes and the full migration window for the intended baseline/cache conditions before choosing it.

### Synchronous packed chunk hashing

`cn-publish -pack-mib 4 -pack-batch-hash` (library `Options.PackBatchHash`) enables synchronous SHA-256 batches of at most 16 independent memory chunks. It defaults to false and requires packing; invalid use fails before taking a publication claim or writing state or remote objects. It applies to both pack identities and preserves every source chunk digest comparison, pack identity, reuse policy, payload budget and INDEX-last publication. Read failures, mismatches and cancellation fail publication; cancellation is checked between bounded batches. The existing independent pack payload remains alive for HTTP body consumption and is not returned to a buffer pool.

On supported x86 CPUs and OS configurations the batch implementation uses AVX512; otherwise it uses the standard library. Each call finishes reading its inputs before returning and starts no background workers. This changes verification computation only, not checkpoint format or durability. Hash microbenchmarks do not establish checkpoint, publish or restore speedups; measure the full operation before enabling this experimental option.

### Conditional operations from checkpoint-restore

`checkpoint-restore --action checkpoint --expected-generation GENERATION` calls `CheckpointIfGeneration`; `--action delete` with the same flag calls `DeleteIfGeneration` and requires the returned `retired_generation` to match exactly. Use the server-assigned `resource_generation` from the original successful Start response. The value is not a checkpoint ID or a label chosen by the client.

A nonempty expectation must contain non-whitespace bytes and be at most 256 bytes; it is compared without trimming. The flag is rejected for start and restore. When omitted or empty, the existing unconditional CLI behavior is retained. Conditional errors, including `Unimplemented` from older servers, never trigger unconditional fallback. An unknown or timed-out request still requires reconciliation; this flag does not implement a durable migration operation or cross-node fencing. `cn-migrate` feeds both flags from one captured value: it strictly parses `sbox inspect` for the migrated sandbox as a single `runtime.SandboxStatus` (the omitted proto3-zero state field is a legal RUNNING), requires the exact id plus a non-blank generation label before checkpointing, and honors an optional `-source-generation` that must match that capture; its source retirement has no fallback — a failed conditional delete is reported as `source-cleanup-pending`, never replaced by an unconditional delete or an empty-listing observation.


The migration entry point limits captured and explicit source generations to 128 bytes, the common limit of checkpoint and retirement; malformed explicit values fail before any node command. The lower-level CLI allows up to 256 bytes, but the retirement RPC rejects values above 128. The captured generation is returned in the migration report and never refreshed for source cleanup. This protects the named source incarnation; it does not provide a durable migration operation, target operation identity, or cross-node writer fencing. A completed receipt may be replayed without touching a replacement incarnation.
